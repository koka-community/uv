...
static void kk_uv_fs_open(kk_string_t path, int32_t flags, int32_t mode, kk_function_t cb, kk_context_t* _ctx) {
  malloc_noinit_request(uv_fs, req, cb);

  int status;
  kk_uv_setup_callback(uv_fs, req, status, cb,
    uv_fs_open(uvloop(), &req->uv, kk_string_cbuf_borrow(path, NULL, _ctx), flags, mode, kk_std_os_file_open_cb),
    { // on_error
