...
// TODO: update WASM API to match below C API
...
void kk_uv_timer_unit_callback(uv_timer_t* uv_timer) {
  kk_uv_hnd_resolve_callback((uv_handle_t*)uv_timer, NULL, kk_get_context());
}
...
kk_std_core_exn__error kk_uv_timer_set_timeout(int64_t timeout, kk_function_t callback, kk_context_t* _ctx) {
  ...
  int status = uv_timer_init(uvloop(), t);
  t->data = NULL;
  kk_box_t internal = kk_cptr_raw_box(&kk_handle_free, t, _ctx);
  kk_uv_timer__timer timer = kk_uv_timer__new_Timer(internal, _ctx);
  kk_uv_check_bail_drops(status, {kk_uv_timer__timer_drop(timer, _ctx);})

  kk_set_hnd_cb(uv_timer_t, kk_uv_timer__timer_dup(timer, _ctx), t, callback);
  status = uv_timer_start(t, kk_uv_timer_unit_callback, timeout, 0 /* no repeat */);
  kk_uv_check_bail_drops(status, {kk_uv_timer__timer_drop(timer, _ctx);})

  // return the timer to the caller, so they can cancel it
  return kk_std_core_exn__new_Ok(kk_uv_timer__timer_box(timer, _ctx), _ctx);
}

kk_std_core_exn__error kk_uv_timer_clear_timeout(kk_uv_timer__timer timer, kk_context_t* _ctx) {
  uv_timer_t* t = kk_owned_handle_to_uv_handle(uv_timer_t, timer);
  int status = uv_timer_stop(t);

  // uv_timer_stop will cancel a pending callback
  kk_uv_hnd_abort_callback((uv_handle_t*)t, _ctx);
...
kk_std_core_exn__error kk_timer_set_timeout(int64_t timeout, kk_function_t callback, kk_context_t* _ctx) {
  #ifdef __EMSCRIPTEN__
    return kk_wasm_timer_set_timeout(timeout, callback, _ctx);
  #else
    return kk_uv_timer_set_timeout(timeout, callback, _ctx);
  #endif
}

kk_std_core_exn__error kk_timer_clear_timeout(kk_uv_timer__timer timer, kk_context_t* _ctx) {
  #ifdef __EMSCRIPTEN__
    return kk_wasm_timer_clear_timeout(timer, _ctx);
  #else
    return kk_uv_timer_clear_timeout(timer, _ctx);
  #endif
}
