...
kk_std_core_exn__error kk_uv_timer_set_timeout(int64_t timeout, kk_function_t callback, kk_context_t* _ctx) {
  KK_ALLOC_AND_BOX_HANDLE(timer);
  int status = uv_timer_init(uvloop(), timer_uvhnd);
  kk_uv_check_bail_drops(status, {kk_box_drop(timer, _ctx);})

  kk_assign_uv_callback(kk_box_dup(timer, _ctx), callback, _ctx);
  status = uv_timer_start(timer_uvhnd, kk_uv_timer_unit_callback, timeout, 0 /* no repeat */);
  kk_uv_check_bail_drops(status, {kk_box_drop(timer, _ctx);})
  ...
  return kk_std_core_exn__new_Ok(boxed_timer, _ctx);
}
...
kk_std_core_exn__error kk_uv_timer_clear_timeout(kk_uv_timer__timer timer, kk_context_t* _ctx) {
  ...
  kk_drop_pending_uv_callback((uv_handle_t*)t, _ctx);
