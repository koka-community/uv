...
static void kk_uv_write_callback(uv_write_t* write, int status){
  kk_context_t* _ctx = kk_get_context();
  kk_uv_handle_t* kk_hnd = uv_write_as_kk_handle(write);
  kk_function_t callback = kk_uv_handle_take_callback(kk_hnd);
...
static void kk_uv_write(kk_uv_stream__uv_stream stream, kk_std_core_types__vector bufs, kk_function_t cb, kk_context_t* _ctx){
  malloc_noinit_request(uv_write, write, cb);
  kk_uv_handle_t* uv_hnd = kk_uv_write_as_handle(write);
