...
static kk_uv_status_dash_code__uv_status_code kk_uv_status_to_status_code(int32_t status,
                                                       kk_context_t *_ctx) {
  switch (status) {
  case 0:
...
kk_std_core_exn__error kk_uv_error(int32_t err, kk_context_t* _ctx) {
  ...
  return kk_std_core_exn__new_Error( kk_std_core_exn__new_Exception(msg, kk_uv_status_dash_code__new_AsyncExn(kk_reuse_null, 0, code, _ctx), _ctx), _ctx );
}
